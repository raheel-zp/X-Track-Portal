...
(function (factory) {
	...
	Listtopie.prototype.run = function () {
		var _ = this;
		_.drawSector(_.itemArr[0].per / 100, _.startDeg, _.options.useMenu === true ? _.itemArr[0].mv : _.itemArr[0].nm, _.itemArr[0].n, _.itemArr[0].c, _.itemArr[0].nm, );
	};

	//main drawing function
	Listtopie.prototype.drawSector = function (percent, start, val, content, color, title) {
		/**
		...
		Snap.animate(0, percent * 359.99, function (deg) {
				...
				var attr = {
					...
					fillOpacity: _.options.drawType === 'round' ? 1 : 0,
			...
			function () {
				...
				if (_.proDraw !== _.itemCount) {
					...
					_.drawSector(_.itemArr[_.proDraw].per / 100, _.startDeg, _.options.useMenu === true ? _.itemArr[_.proDraw].mv : _.itemArr[_.proDraw].nm, _.itemArr[_.proDraw].n, _.itemArr[_.proDraw].c);
					...
					if (_.options.drawType === 'simple') {
						_.$sectors.forEach(function (el) {
							...
							_.$gText.select('#t' + _.$snap.id + el.attr('id')).attr({
								'opacity': '1'
							});
						});
					}
	...
	Listtopie.prototype.animateChange = function (ang, angText, angTextCheck) {
		var _ = this;
		// rotate sectors
		Snap.animate(_.transformAngle, ang, function (ang) {
				_.$g.transform('r' + ang + "," + _.$svgCenter + "," + _.$svgCenter)
			}, _.options.speedRotate, _.options.easingType,
			function () {
		...
		Snap.animate(_.transformAngle, ang, function (ang) {
			_.$gText.transform('r' + ang + "," + _.$svgCenter + "," + _.$svgCenter)
		}, _.options.speedRotate, _.options.easingType);
	...
	Listtopie.prototype.rotateText = function (angleVal, check) {
		...
		if (check === true) {
			_.$snap.selectAll("text").forEach(function (t) {
				...
				var ty = t.attr("ty");
				t.animate({
					transform: "r" + (angleVal - _.options.startAngle) + "," + tx + ',' + ty
				}, 200);
			});
		} else if (check === false) {
			_.$snap.selectAll("text").forEach(function (t) {
